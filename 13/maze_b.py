...
for x in range(rows):
    for y in range(cols):
        if maze[x][y]==0:
            path = run_bfs(grid, start, (x,y))
            length = path_length(start, (x,y), path)
            if 0 < length <= 50:
                print("End = ({},{}) Length = {}".format(x, y, length))
                locations += 1

# add one extra to count starting position (only valid node with length==0)                
print("Distinct locations = {}".format(locations+1))
